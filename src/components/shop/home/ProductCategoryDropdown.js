...
const CategoryList = () => {
  ...
  return (
    <div className={`${data.categoryListDropdown ? "" : "hidden"} absolute z-20 top-10 left-0 w-full shadow-lg bg-white border border-gray-200 rounded-b-lg p-4`}>
            ...
            return (
              <Fragment key={index}>
                <div
                  ...
                  className="col-span-1 m-2 flex flex-col items-center justify-center space-y-1 cursor-pointer hover:bg-gray-100 p-2 rounded transition-colors duration-200"
                >
                  <img
                    ...
                    alt="pic"
                  />
                  <div className="font-medium text-sm text-center truncate w-full">{item.cName}</div>
                </div>
              </Fragment>
            );
          })
        ...
        <div className="flex justify-between items-center">
          <div className="flex flex-col space-y-2  w-2/3 lg:w-2/4">
            <label htmlFor="points" className="text-sm">
              Price (between 0 and 10.000.000 Rp):{" "}
              <span className="font-semibold text-yellow-700">{range}.000 Rp</span>{" "}
            </label>
            <input
              value={range}
              className="slider"
